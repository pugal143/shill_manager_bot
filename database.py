...
def dict_add(update, text):
    ...
    myquestion.insert_one({"question":"Hello"})
...
def sumbit_request(update , bot , username , chat_id):
    if check_per(username) == "allow":
        update.message.reply_text("🔽Now click send your message link🔽")
    else:
...
def task_submit(update , bot , username , chat_id ,text):
    ind_time_date = datetime.now(timezone("Asia/Kolkata")).strftime('%d')
